...
object Solution {
  ...
  */
   ...
   //2 2 8
    def minimumBribes(q: Array[Int]) = calculateNumberOfBribes(q.toList.zipWithIndex, 0).getOrElse("Too chaotic")

    private def calculateNumberOfBribes(q: List[(Int, Int)], total: Int): Option[Int] = q match {
        case head :: tail if(isNumberInIllegalPosition(head)) => None 
        case head :: tail if(isNumberInOriginalPosition(head)) => calculateNumberOfBribes(tail, total)
        case head :: tail if(!isNumberInOriginalPosition(head)) => calculateNumberOfBribes(tail, total + calulateNumbersBribe(head)) 
        case Nil => Some(total)
    }

    private def isNumberInOriginalPosition(numberAndPosition: (Int, Int)) = numberAndPosition match { 
    ...
    private def isNumberInIllegalPosition(numberAndPosition: (Int, Int)) = {
      ...
      val isIllegal = numberAndPosition match {
         case (number, index) => number - adjustArrayIndexToNaturalNumber(index) > maxBribes
      }
