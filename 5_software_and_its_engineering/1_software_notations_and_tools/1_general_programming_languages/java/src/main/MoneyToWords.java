...
class MoneyToWords {
    ...
    public static String parseDecimal(String userNumber, int decimalPoint) {
        //Move to function
        //arguments: userinput. decimalPoiint

        String decimal = userNumber.substring(decimalPoint + 1);
        String decimalConverion = "";
        int length = decimal.length();
        switch (length) {
            case 2:
                if ((Integer.parseInt(decimal)) >= 1) {
                    decimalConverion = convertTwoNumbers(Integer.parseInt(decimal));
                }
                break;
            case 1:
                decimalConverion = tens[Integer.parseInt(decimal)];
                break;
            default:
                decimalConverion = "";
        }
        return decimalConverion;
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (num == 0 && decimalPoint == -1) {
            ...
            String temp = parseDecimal(userNumber, decimalPoint);
            if (temp == "") {
                System.out.println("Zero" + Currency + and + "Zero" + Cents);
            } else {
            ...
            while (num > 0) {
                ...
                switch (length) {
                    ...
                    case 4:
                        ...
                        num = remainder;
                        if (num == 0) 
                           WordVersion = ltr + " " + places[2] + Currency;
                        else
            ...
            if (decimalPoint != -1) {
                ...
                String temp = parseDecimal(userNumber, decimalPoint);
                if (!temp.equals("")) {
                    WordVersion += and + parseDecimal(userNumber, decimalPoint) + Cents;
                }
            }
        }
