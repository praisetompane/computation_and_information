    ...
    for col in range(width):
        for row in range(height):
            ...
            regAvg = (
                ...
                + BottomRightPixel.getRed()
            ) / 4
            greenAvg = (
                startPixel.getGreen()
                + topLeftPixel.getGreen()
                + topRightPixel.getGreen()
                + BottomRightPixel.getGreen()
            ) / 4
            blueAvg = (
                startPixel.getGreen()
                + topLeftPixel.getBlue()
                + topRightPixel.getBlue()
                + BottomRightPixel.getBlue()
            ) / 4
...
def doubleImageSize(imageToChange, scaleFactor):
    ...
    for col in range(width):
        for row in range(height):
            ...
            newImage.setPixel(
                (col * scaleFactor) + 1, (row * scaleFactor) + 1, currentPixel
            )
