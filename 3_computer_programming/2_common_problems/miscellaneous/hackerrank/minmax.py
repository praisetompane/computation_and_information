def birthdayCakeCandles(n, ar):
    ...
    def countAppearance(appearanceTotal, index):
        nextIndex = index + 1
        if index == length: return appearanceTotal
        elif ar[index] == maxHeight: return countAppearance(appearanceTotal + 1, nextIndex)
        else: return countAppearance(appearanceTotal, nextIndex)
...
assert(birthdayCakeCandles(4, [3, 2, 1]) == 1) #note the age and ar elements are not the same, for sneaky input
